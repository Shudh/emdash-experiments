...
import { defaultWorkflowDefinition, workflowDefinitionById } from "../definitions/registry.js";
import {
	WF_ASSET_STATE,
	WF_CARD_STATE,
	WF_STATUS,
...
export function workflowDefinitionForInstance(instance: DomainRow): WorkflowDefinition {
	return workflowDefinitionById(
		asString(instance.definition_id),
...
export async function getInstanceOrThrow(
	store: DomainStore,
	instanceId: string,
): Promise<DomainRow> {
...
export async function createInitialWorkflowInstance(
	...
	const definition = input?.definitionId
		? workflowDefinitionById(input.definitionId, input.definitionVersion ?? 1)
		: workflowDefinition();
...
export async function createInitialWorkflowCards(
	...
	const definition = workflowDefinitionForInstance(input.instance);
