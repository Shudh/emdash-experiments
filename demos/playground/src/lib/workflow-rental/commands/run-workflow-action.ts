...
function isRequiredCard(card: DomainRow): boolean {
...
async function unresolvedRequiredCardsForInstance(
	...
	return cards.filter((card) => isRequiredCard(card) && isUnresolvedCard(card));
}
...
export async function runWorkflowAction(
	...
	return store.transaction(async (tx) => {
		...
		const unresolvedRequiredCards = await unresolvedRequiredCardsForInstance(tx, instance.id);

		if ((action.unresolvedCardPolicy ?? "allow") === "block" && unresolvedRequiredCards.length > 0) {
