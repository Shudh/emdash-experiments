...
import {
	...
	rentalRequest,
	uniqueRentalTitle,
	type LocalRentalActor,
} from "../rental-flow-utils.js";

async function loginAs(browser: Browser, email: string): Promise<LocalRentalActor> {
	return loginAsLocalRentalUser(browser, email);
}
...
test("local persistent users run full rental lifecycle without passkey recreation", async ({
	...
	try {
		const title = uniqueRentalTitle("Habitat Mayflower API Flow");
